...
spp <- c('carinata', 'elongata', 'sublineata', 'carinulata', 'elongata_sub', 'carinu_sub', 'extra')
...
## Pull pure individuals for reference database. 
...
pCS <- ggplot(car_sub_hyb) +
  ...
  geom_histogram(aes(sublineata), 
                 alpha = .2, fill = legend[which(legend$spp=="sublineata"), "cbbPalette"], stat = "bin", binwidth = .05) +
  ylim(c(0,8)) +
  scale_x_continuous(name= "D. carinata X D. sublineata q-values") +
  theme_bw()

pCE <-ggplot(car_elong_hyb) +
  geom_histogram(aes(carinata), 
                 alpha = .2, fill = legend[which(legend$spp=="carinata"), "cbbPalette"], stat = "bin", binwidth = .05) +
  geom_histogram(aes(elongata), 
                 alpha = .2, fill = legend[which(legend$spp=="elongata"), "cbbPalette"], stat = "bin", binwidth = .05) +
  ylim(c(0,8)) +
  scale_x_continuous(name= "D. carinata X D. elongata q-values") +
  theme_bw()
...
pES <-ggplot(elong_sub_hyb) +
  ...
  geom_histogram(aes(sublineata), 
                 alpha = .2, fill = legend[which(legend$spp=="sublineata"), "cbbPalette"], stat = "bin", binwidth = .05) +
  ylim(c(0,8)) +
  scale_x_continuous(name= "D. sublineata X D. elongata q-values") +
  theme_bw()
  
cowplot::plot_grid(pCS, pCE, pES)
...
pure_parentals$assignment <- colnames(pure_parentals[,12:15])[max.col(pure_parentals[,12:15])]
...
elongata_popmap <- left_join(data.frame(sampID = rownames(slist[[1]])), 
                    popmap)

tail(popmap)

popID <- data.frame(popID = elongata_popmap$popID)
popID <- data.frame(lapply(popID, as.character), stringsAsFactors=FALSE)
sapply(popID, is.character)

pop_order <- sample_meta.dat[,c("popID", "Order")]
elongata_pop_order <- semi_join(pop_order, popmap)
pop_order <- arrange(elongata_pop_order, Order)[,1]
```
