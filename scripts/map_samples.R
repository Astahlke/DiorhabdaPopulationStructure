...
sample_meta.dat <- read.csv("Git_Repo/pop_metadata.csv", 
                            header = TRUE, stringsAsFactors = FALSE)

sample_meta.dat[22:37,]$Grp <- "hybrid_zone"
...
NAwater <- watershedDF %>%
  filter(long < -95 & long >-120 & lat > 25 & lat <44.8)
...
NAsamples <- sample_meta.dat %>%
  filter(Longitude < -95 & Longitude >-120 & Latitude > 25 & Latitude <50)
